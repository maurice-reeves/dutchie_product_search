...
def product_detail(product_row: int):
    ...
    try:
        ...
        if grp and primary:
            ...
            rows = conn.execute("""
                SELECT p.id, p.name, p.display_name, p.brand_name, p.product_type, p.product_subcategory, p.weight_label,
                       p.image_url, p.dispensary_display, p.dispensary_slug,
        ...
        similar, seen = [], set()
        for r in conn.execute("""
            SELECT p.id, p.name, p.display_name, p.brand_name, p.product_type, p.product_subcategory, p.weight_label, p.thc_display, p.price,
